...
export default async function RulesPage() {
  ...
  return (
    <PageContainer>
      ...
      />
      <p className="mb-3 max-w-2xl font-mono text-xs uppercase tracking-widest text-dim">
        {t.rules.heroSubtitle}
      </p>
      <div className="mb-4 flex flex-wrap gap-2">
        <Badge variant={isAuto ? "amber" : "dim"}>
          {isAuto ? t.rules.autoBadge : t.rules.manualBadge}
        </Badge>
        <Badge variant="neutral">{season.title}</Badge>
        <Link
          href="/board"
          className="border border-amber/30 px-2 py-0.5 font-mono text-[11px] uppercase tracking-widest text-amber hover:bg-amber hover:text-black"
        >
          {t.rules.viewBoard}
        </Link>
        <Link
          href="/leaderboard"
          className="border border-dim/20 px-2 py-0.5 font-mono text-[11px] uppercase tracking-widest text-dim hover:text-amber"
        >
      ...
      {isAuto ? (
        ...
        <div className="space-y-4">
          <div className="hud-card animate-hud-rise border-amber/20 bg-amber/5 px-4 py-2 font-mono text-xs text-dim">
            <Badge variant="dim" size="sm">
              {t.rules.manualBadge}
            </Badge>{" "}
            <span className="ml-2">{t.rules.manualHint}</span>
          </div>
          <div className="hud-card animate-hud-rise p-6 sm:p-8" style={{ animationDelay: "60ms" }}>
            <Markdown source={season.rulesMd} />
          </div>
        </div>
      ) : (
        <div className="hud-card animate-hud-rise p-10 text-center">
