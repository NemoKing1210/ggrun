...
export async function updateProviderKeysUseCase(input: unknown) {
  ...
  const { resetProxyAgent } = await import("@/lib/infrastructure/http/external-fetch");
...
export async function testProxyUseCase(proxyUrl: string): Promise<{ ok: boolean; error?: "http" | "network"; status?: number; detail?: string }> {
  ...
  if (url === "__USE_CURRENT__") {
    const effective = await (await import("@/lib/infrastructure/http/external-fetch")).getEffectiveProxy();
    url = effective.url ?? "";
  }
