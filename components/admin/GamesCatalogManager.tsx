  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="hud-card p-5">
        <h2 className="inline-flex items-center gap-2 font-display text-lg uppercase tracking-wider">
          <span className="inline-flex size-7 items-center justify-center bg-amber/10 border border-amber/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)]">
            <PlusIcon className="size-4 text-amber" aria-hidden />
          </span>
      ...
      <section className="hud-card p-5">
        <h2 className="inline-flex items-center gap-2 font-display text-lg uppercase tracking-wider">
          <span className="inline-flex size-7 items-center justify-center bg-amber/10 border border-amber/30 [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)]">
            <MagnifyingGlassIcon className="size-4 text-amber" aria-hidden />
          </span>
          {t.admin.catalog.searchHeading}
        </h2>
        <p className="mt-2 text-xs leading-relaxed text-zinc-500">{t.admin.catalog.searchHint}</p>
        <div className="hazard-tape my-3 opacity-60" aria-hidden />
        <form action={searchAction} className="grid grid-cols-1 sm:grid-cols-5 gap-3 items-end">
          ...
          <button type="submit" disabled={searchPending} className="hud-btn hud-btn-primary sm:col-span-5 inline-flex items-center justify-center gap-2">
        ...
        {searchState.results && (
          <div className="mt-4 grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {searchState.results.length === 0 ? (
              <div className="hud-card p-6 border-dashed flex flex-col items-center gap-2 text-center sm:col-span-3">
              ...
              searchState.results.map((r) => (
                <div
                  ...
                  className="hud-card hud-lift p-3 bg-[#0f0f0f] flex gap-3 [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)]"
                >
                  <div className="w-16 h-20 bg-zinc-800 overflow-hidden shrink-0 border border-[#2a2a22] [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)]">
                    {r.coverUrl ? (
      ...
      <section className="hud-card p-0 overflow-hidden">
        <div className="flex flex-wrap items-center justify-between gap-3 border-b border-[#3d3d34] bg-raised/40 p-4">
          <h2 className="inline-flex items-center gap-2 font-display text-lg uppercase tracking-wider">
            <span className="inline-flex size-7 items-center justify-center bg-raised border border-[#3d3d34] [clip-path:polygon(4px_0,100%_0,100%_calc(100%-4px),calc(100%-4px)_100%,0_100%,0_4px)]">
              <FunnelIcon className="size-4 text-amber" aria-hidden />
            </span>
            {format(t.admin.catalog.poolHeadingFull, { shown: filtered.length, total: games.length })}
          </h2>
          <div className="flex items-center gap-2">
            <span className="hidden font-mono text-[10px] uppercase tracking-widest text-dim sm:inline">Filter</span>
            <div className="w-56">
        ...
        {filtered.length === 0 ? (
          <div className="m-4 border border-dashed border-dim/20 bg-background/20 p-8 text-center [clip-path:polygon(6px_0,100%_0,100%_calc(100%-6px),calc(100%-6px)_100%,0_100%,0_6px)]">
            <CircleStackIcon className="mx-auto size-6 text-dim" aria-hidden />
            <p className="mt-2 font-mono text-xs uppercase tracking-widest text-dim">No games match filter</p>
            <p className="mt-1 text-sm text-zinc-500">Try a different search term or clear the filter.</p>
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10 bg-raised text-dim text-left border-b border-[#3d3d34]">
                <tr>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colCover}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colTitle}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colPlatform}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colGenres}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colMeta}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colStatus}</th>
                  <th className="p-2 font-mono text-[11px] uppercase tracking-widest">{t.admin.catalog.colActions}</th>
                </tr>
              </thead>
              <tbody>
                {filtered.map((g) => (
                  <tr key={g.id} className="border-b border-[#2a2a22] hover:bg-amber/[0.04] transition-colors">
                    <td className="p-2">
                      <div className="w-10 h-10 overflow-hidden bg-zinc-800 border border-[#2a2a22] [clip-path:polygon(3px_0,100%_0,100%_calc(100%-3px),calc(100%-3px)_100%,0_100%,0_3px)]">
                        {g.coverUrl ? (
                          // eslint-disable-next-line @next/next/no-img-element
                          <img src={g.coverUrl} alt="" className="w-full h-full object-cover" />
                        ) : (
                          <div className="grid place-items-center h-full text-[10px] text-dim">
                    ...
                    <td className="p-2 max-w-[220px]">
                      <span className="font-display text-sm uppercase tracking-wide line-clamp-1">{g.title}</span>
                      {g.tags && g.tags.length ? (
                        <div className="flex gap-1 mt-1 flex-wrap">
                          {g.tags.slice(0, 3).map((tg) => (
                            <Badge key={tg} variant="dim" size="sm">
                              {tg}
                            </Badge>
                          ))}
                        </div>
                    ...
                    <td className="p-2 font-mono text-xs">
                      {g.platform ? <Badge variant="neutral" size="sm">{g.platform}</Badge> : <span className="text-dim">—</span>}
                    </td>
                    <td className="p-2">
                    ...
                    <td className="p-2">
                      <div className="flex items-center gap-1">
                        ...
                        {g.rating ? (
                          <span className="ml-1 inline-flex items-center gap-1 text-zinc-400 text-xs">
                    ...
                    <td className="p-2">
                      <div className="flex gap-2">
                        <form action={toggleBlacklistAction}>
                          ...
                          <button type="submit" className="hud-btn !py-1 !px-2 text-xs border-dim/30">
                            {g.isBlacklisted ? t.admin.catalog.unblockButton : t.admin.catalog.blockButton}
                          </button>
                        </form>
                        <form action={deleteGameAction}>
                          <input type="hidden" name="gameId" value={g.id} />
                          <button type="submit" className="hud-btn hud-btn-danger !py-1 !px-2 text-xs">
