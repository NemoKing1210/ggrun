...
export function GamesHistory({ rolls }: { rolls: HistoryRoll[] }) {
  ...
  return (
    <>
          ...
          {rolls.map((roll) => (
            ...
            >
              <div className="flex items-start justify-between gap-3">
                ...
                <span
                  className={`shrink-0 border px-2 py-1 font-mono text-[10px] uppercase tracking-widest [clip-path:polygon(3px_0,100%_0,100%_calc(100%-3px),calc(100%-3px)_100%,0_100%,0_3px)] ${statusTone(roll.status)}`}
                >
                  {roll.status}
                </span>
              </div>
              ...
              {roll.notes ? (
                <p className="mt-3 line-clamp-3 border-l-2 border-amber/25 pl-3 text-sm leading-relaxed text-zinc-300">
                  “{roll.notes}”
                </p>
              ) : null}
      ...
      <GameDetailsModal
        game={
          current?.game ? toGameDetails(current.game as unknown as Record<string, unknown>) : null
        }
